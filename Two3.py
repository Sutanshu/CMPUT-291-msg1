...
def search_for_sales(pid = None,user=None):
    global connection, cursor
    ...
    while repeat:
        ...
        if not pid is  None:
            query_get_active_sales = """
                select s.sid, s.descr, max (amount), s.rprice, cast(julianday(s.edate)-julianday('now') as int)
                from sales s left outer join bids b using (sid)
                where s.edate > datetime('now') and s.pid = ?
                group by sid, s.descr, s.edate;
                """
            cursor.execute(query_get_active_sales,(pid,))
    ...
    choose_action_for_sale(sale_chosen, max_amt, row,user)

def choose_action_for_sale(sale_chosen, max_amt, row,user):
    """
    ...
    while repeat:
        ...
        if action == "1":
            place_bid(sale_chosen[1][0], max_amt,user)
        elif action == "2":
            list_active_sales_of_seller(row[0],user)
        elif action == "3":
            list_reviews(row[0])
        else:
...
def place_bid(sale_id, max_amt_arg,user):
    """
    ...
    insert_bid(sale_id, amt,user)

def insert_bid(sid, bid_val,user):
    """
    ...
    cursor.execute(insert_bid_query, (str(bid_num), user, sid, time.strftime("%Y-%m-%d"), bid_val))

    connection.commit()

def list_active_sales_of_seller(seller,user):
    """
...
def newfunc(pid=None,user=None,path = None):
    global connection, cursor

    connect(path)
    if not pid is None:
        search_for_sales(pid,user)
    else:
        search_for_sales(user=user)
